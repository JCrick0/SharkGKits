...
public class InventoryUtil {
    ...
    public static String serializeInventory(ItemStack[] source) {
        ...
        //List<ItemStack> items = Arrays.stream(source).map(s -> Objects.isNull(s) ? ItemMaker.of(Material.AIR).build() : s).collect(Collectors.toList());
        Arrays.stream(source).collect(Collectors.toList()).replaceAll(t -> Objects.isNull(t) ? ItemMaker.of(Material.AIR).build() : t);
