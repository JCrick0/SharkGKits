package net.sharkdevelopment.lib.menu;

import net.sharkdevelopment.gkits.SharkGKits;
import net.sharkdevelopment.lib.menu.pagination.PaginatedMenu;
...
public class ButtonListener implements Listener {
        ...
        if (openMenu != null) {
            ...
            if (openMenu.getButtons().containsKey(event.getSlot())) {
                ...
                if (event.isCancelled()) {
                    Bukkit
                            .getScheduler()
                            .runTaskLater(SharkGKits.getInstance(), player::updateInventory, 1L);
                }
            } else if (
                    event.getClick() == ClickType.SHIFT_LEFT ||
                            event.getClick() == ClickType.SHIFT_RIGHT ||
    ...
    public void onInventoryClose(InventoryCloseEvent event) {
        ...
        player.setMetadata("scanglitch", new FixedMetadataValue(SharkGKits.getInstance(), true));
    }
    ...
    public void onPlayerMove(PlayerMoveEvent event) {
        ...
        if (player.hasMetadata("scanglitch")) {
            player.removeMetadata("scanglitch", SharkGKits.getInstance());
            ...
            for (ItemStack it : player.getInventory().getContents()) {
                if (it != null) {
                    ...
                    if (
                            meta != null &&
                                    meta.hasDisplayName() &&
                                    meta.getDisplayName().contains("§b§c§d§e")
                    ) {
